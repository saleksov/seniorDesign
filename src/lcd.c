...
//===========================================================================
void LCD_Clear(u16 Color, int x1, int y1, int x2, int y2)
{
    lcddev.select(1);
    unsigned int i,m;
...
void _LCD_DrawChar(u16 x,u16 y,u16 fc, u16 bc, char num, u8 size, u8 mode)
{
    ...
    u16 y_draw;
    num=num-' ';
    LCD_SetWindow(y,x,y+75,x+50); //setting window to print the character
        for(pos=0;pos<size;pos++)
        {
            ...
            temp = tempy;
            y_draw = y + pos * SCALE + yscale;
                for (t=size/2;t>0;t--)
                {
                    if(temp&0x01){
                        for(int xscale = 0; xscale < SCALE; xscale++){
                            x_draw = x + t * SCALE + xscale;
                            _LCD_DrawPoint(y_draw-10, x_draw-5, fc);
                        }
                    }
                    temp>>=1;
                }
